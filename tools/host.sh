...
#   linux | macos | windows | ios | ios-simulator | android
...
fun_triple() {
    case "$(fun_target)" in
        ...
        ios-simulator)
            ...
            ;;
        android|aarch64-android|arm-android|x86_64-android|x86-android)
            case "$(fun_android_abi)" in
                arm64-v8a) printf '%s\n' aarch64-android ;;
                armeabi-v7a) printf '%s\n' arm-android ;;
                x86_64) printf '%s\n' x86_64-android ;;
...
# Dawn's bundled protobuf cannot build protoc for iOS/Android targets.
...
fun_can_run_native() {
    ...
    # Android NDK binaries cannot execute on the build host.
    if fun_is_ios || fun_is_android; then
        return 1
    fi
    return 0
}
